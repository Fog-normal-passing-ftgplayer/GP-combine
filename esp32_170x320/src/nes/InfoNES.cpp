...
/*-------------------------------------------------------------------*/
/*  GP-Combine: 帧结束标志                                            */
/*  原版 InfoNES_Cycle() 是阻塞式 for(;;) 循环，只在退出时返回。        */
/*  直接放进 Arduino loop() 会让 loop 任务永不返回 -> 看门狗复位。      */
/*  这里每渲染完一帧就返回一次，由固件按自己的节奏驱动。                  */
/*-------------------------------------------------------------------*/
int InfoNES_FrameReady;
 ...
 */
  ...
  {    
    ...
    // GP-Combine: 一帧已渲染并推屏，交还控制权给固件 loop()
    if ( InfoNES_FrameReady )
    {
      InfoNES_FrameReady = 0;
      return;
    }
  ...
  switch ( PPU_Scanline )
  {
    ...
    case SCAN_UNKNOWN_START:
      if ( FrameCnt == 0 )
      {
        ...
        // GP-Combine: 标记帧结束，让 InfoNES_Cycle() 返回
        InfoNES_FrameReady = 1;
