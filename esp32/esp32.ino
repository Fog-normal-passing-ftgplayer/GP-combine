...
void drawMenuBg() {
  if (curThemeImageBg) {
#if WALLPAPER_ACTIVE
    if (!gifDrawCurrent()) memcpy(lfb, BACKGROUND_IMG[bgOpts[0].value], 240 * 135 * 2);
#else
    ...
    for (int i = first; i < sec.count && i < first + 5; i++) {
      ...
      if (o->type == OPT_SLIDER) {
        ...
        if (i == subSel) {
          ...
          if (locked) drawCJKText(214 - 32, y, "关闭", ACC_SETTINGS, 1);
          else drawOptValue(o, editable ? 214 : 226, y, ACC_SETTINGS);
          ...
          if (locked) drawCJKText(214 - 32, y, "关闭", colText, 1);
          else drawOptValue(o, editable ? 214 : 226, y, colText);
...
#if WALLPAPER_ACTIVE
    if (subPage == 4) {
      drawCJKTextCentered(120, 122, "动态壁纸模式 · 屏保已禁用", RGB565(120,132,150), 1);
    } else if (subPage == 3) {
      drawCJKTextCentered(120, 122,
