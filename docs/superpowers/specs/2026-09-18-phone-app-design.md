...
| 项 | 数据 | 来源 |
|---|---|---|
| app 分区占用 | 825,118 B / 4,194,304 B（含 BLE 协议栈；没接蓝牙时 572,339 B） | `arduino-cli compile` |
| 内部 RAM 静态占用 | **78,248 B** / 327,680 B（`lfb`、`rbuf` 都已不在 `.bss`） | `arduino-cli compile` |
| 内部堆（蓝牙关，开机稳定后） | free **131,464 B**，最大连续块 **90,100 B**，DMA **123,704 B** | 设备实测（`[heap]` 调试输出） |
| 内部堆（蓝牙开、已广播） | free **67,088 B**，最大连续块 **31,732 B** | 同上 |
| RAM 大头 | `lfb` 108,800（渲染目标）+ `rbuf` 108,800（DMA 源），合计 212.5 KB | `nm --size-sort` |
| PSRAM | 8 MB 里用掉约 2.6 MB：`lfb` 108.8 KB + 动态壁纸 GIF 2.54 MB | 设备实测 |
...
1. ~~先把 `rbuf` 挪到 PSRAM 腾内部 RAM~~ —— **2026-09-18 实测否掉了这条路**：PSRAM 版 `rbuf` 对内部堆的开销是 0，但运行后内部堆 `free=61,440 / alloc=164,160`，比内部版（`free=63,244 / alloc=162,420`）还少；每帧 `conv` 从 6.5 ms 涨到 8.4 ms。原因是 SPI 主控驱动对非 DMA 内存的源缓冲会另开一份内部 bounce buffer，等于把 106 KB 又搬回内部。**`rbuf` 留在内部 RAM**。真要再抠内存，就上"分块流水线"：只留 2 块（54,400 B）做双缓冲，边转边发，省 54 KB。

1b. **`rbuf` 不能要求"一整块连续 108,800"**（2026-09-18 踩到的坑）：接入 NimBLE 后协议栈静态数据把内部堆切成 `free=137,100 但最大连续块只有 90,100`，`heap_caps_aligned_alloc(108800, INTERNAL|DMA)` 直接失败；旧代码 `while(true) delay(1000)` 静默死等 —— 表面现象是"刷完黑屏、USB 调试口一个字都没有、设备也不重启"，非常难查。现已改成 **4 块 × 27,200**（和 `lcdSendFrameAsync` 的 DMA 分块本来就一致），内存总量、分块方式、帧率都不变，堆再碎也塞得下；真分配不到会打印 `RBUF FAIL ...` 再重启，不再静默卡死。

1c. **给蓝牙腾内存只能从 `lfb` 下手**（结论）：`esp_bt_controller_init()` 要一大块连续内部 RAM，两个整屏缓冲都放内部时它直接 `Malloc failed`（`-4`）→ 断言 → 复位循环。因为 `rbuf` 挪 PSRAM 不省内存（见第 1 条），改把**只被 CPU 读写、从不参与 DMA 的渲染目标 `lfb`** 挪到 PSRAM：静态 RAM 从 186.5 KB 降到 78.2 KB，蓝牙开关打开时内部堆还剩 67 KB，控制器正常起来。代价是每帧 `conv` 从 6.1 ms 涨到 8.3 ms（读 108.8 KB 的 PSRAM），壁纸帧率不变（15.3 fps），NES/动画会受一点影响。

1d. 顺带记住：`conv` 的扫描顺序（外层面板行、内层面板列）**不要反着写**。反过来的版本读 `lfb` 是连续的，但写 `rbuf` 变成跨 340 字节，写侧 cache 全打穿，实测 8.2 ms → 9.4 ms。
...
- **蓝牙与 nRF 无线手柄输出互斥**（2026-09-18 定）：蓝牙开关打开 ⇒ nRF 模块 `powerDown()` 彻底停发（不是发空包），关掉蓝牙再按「无线开关」这个偏好恢复。无线页标题行在蓝牙模式下显示「蓝牙模式」，蓝牙页底部提示「nRF已关闭 关蓝牙恢复」。**蓝牙开关默认关**——否则默认就把无线手柄功能关了。实现上唯一状态源是 `btOpts[0].value`，`applyWirelessSettings()` 里判断（`btEnabled()`）。顺带记一笔频谱：nRF24 用 channel 100 = **2500 MHz**（`nrf24.h`），BLE 是 2402–2480 MHz，两者**不重叠**，所以互斥主要防的是同壳内两个发射机互相去敏 + core 0 上的任务抢占，而不是抢同一个信道。
...
| 资源 | 现在 | 加完 BLE+WiFi 后 |
|---|---|---|
| Flash（app） | 825 KB（含 BLE 协议栈） | ~1.4 MB（再加 WiFi/HTTP 551 KB + 协议代码） |
| 内部 RAM 静态 | 78,248 B | 基本不变 |
| 内部堆（稳定） | 蓝牙关 131 KB / 蓝牙开 67 KB | 扣掉 WiFi ~50 KB，**余 ~17 KB** |
| PSRAM | `lfb` 108.8 KB + 壁纸 2.54 MB | 再加协议缓冲 8 KB，余量充足 |
...
| BLE 与 nRF 手柄输出互相压制/抢 CPU | 不让它们共存：蓝牙开就 `powerDown()` nRF（互斥），菜单上有明确状态提示 |
