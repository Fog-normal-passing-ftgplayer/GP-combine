...
- **开关走 BLE**：`CMD_WIFI_START (0x30)` 返回 `ssid=…;pass=…;ip=192.168.4.1`；
  `CMD_WIFI_STOP (0x31)` 主动关。60 秒无活动自动关
- **传输期间挂起 BLE**（2026-09-24 实测后定的，见下面的数字）。手机在开始传之前
  已经从 BLE 拿到 SSID/密码，传的时候 BLE 是闲的；挂起它既腾出内部堆，也让 2.4G
  射频不再被 BLE 分时抢走。代价：上传期间手机 BLE 会断，得有重连路径
...
- **传完自动生效**：复用 `gifInit()` 重载
  ✅ **前置项已核实（2026-09-24）**：原计划里"M3 第一个必做项 = 修 `gifLoadFromFs()`
  重载泄漏 PSRAM"是**虚警** —— `if (gifFsBuf) free(gifFsBuf);` 从 `3b1d4b89` 起就在，
  切帧解码路径也没有反复 malloc。这段不用做，直接进 HTTP。

**内存门槛实测（2026-09-24，探针固件在板上量的）**：拿真实基线的堆占用做模拟
（`free=69044 / largest=31732`，即正式固件 BLE 起来后的样子），再开 AP：

| 状态 | 内部堆 free | 最大连续块 |
|---|---|---|
| 模拟正式基线（BLE 在跑） | 69044 | 31732 |
| + `WiFi.softAP()` | 16796 | 8692 |
| + `WebServer.begin()` | 16072 | 7924 |
| 挂起 BLE 后（AP 还开着） | **78032** | **27636** |

结论：**AP 能起来，但不挂 BLE 的话只剩 16 KB 余量**（还要跑 LittleFS 写盘、4 KB 分片、
BLE GATT 通知），太紧；挂起 BLE 能拿回约 62 KB。所以传输期间挂 BLE 是硬要求，
不是优化项。另外本机（开发机）**没有无线网卡**，AP 的 HTTP 表面只能手机侧验：
连接热点后用 Termux 里的 curl 跑一遍端点清单。
