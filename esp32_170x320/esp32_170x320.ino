...
static void espCfgPack(uint8_t *p) {
  p[0] = 1;
  p[1] = ESP_CFG_FORMAT;
  p[2]  = (uint8_t)histOpts[0].value;   // 输入历史
  p[3]  = (uint8_t)histOpts[1].value;   // 按键布局
  p[4]  = (uint8_t)bgOpts[0].value;     // 背景透明度
  p[5]  = (uint8_t)bgOpts[1].value;     // 背光亮度
  p[6]  = (uint8_t)bgOpts[2].value;     // 水平翻转
  p[7]  = (uint8_t)bgOpts[3].value;     // 垂直翻转
  p[8]  = (uint8_t)bgOpts[4].value;     // 反色
  p[9]  = (uint8_t)sleepOpts[0].value;  // 屏保模式
  p[10] = (uint8_t)(sleepOpts[1].value & 0xFF);
  p[11] = (uint8_t)((sleepOpts[1].value >> 8) & 0xFF);
  p[12] = (uint8_t)sleepOpts[2].value;  // 关屏
  p[13] = (uint8_t)wlOpts[0].value;     // 无线开关
  p[14] = (uint8_t)histOpts[2].value;   // 主题
  p[15] = (uint8_t)histOpts[3].value;   // 风格
  p[16] = 0;                            // 保留
}
